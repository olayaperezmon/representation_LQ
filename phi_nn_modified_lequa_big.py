...
class InstanceRepresentationBig(nn.Module):
    """
    This class defines the representation of the individual isntances. 
    In general (phi_l, phi_f, phi_A), it return the next_to_last linar infromation 
    except when we use a traditional classifier (phi_h) that returns the posteriors. 
    This is exactly the same as FCFeatureExtractor in HistNetQ.
    """

    def __init__(self, X_shape, dropout, return_next_to_last, num_classes=None):
        super(InstanceRepresentationBig, self).__init__()
        self.return_next_to_last = return_next_to_last
        self.linear_1 = nn.Linear(X_shape, 1024)
        self.relu_1 = nn.LeakyReLU()
        self.dropout_1 = nn.Dropout(0.5)
        self.next_to_last_linear = nn.Linear(1024, 512)
        if num_classes != None: 
            self.relu_2 = nn.ReLU()
            if num_classes == 2:
...
class InstanceRepresentationBig_decoder(nn.Module):
    def __init__(self, X_shape, last_linear_shape, dropout):
        super(InstanceRepresentationBig_decoder, self).__init__()
        self.module = torch.nn.Sequential()
        self.module.add_module("linear_1_dec", nn.Linear(last_linear_shape, 1024))
        self.module.add_module("relu_1_dec", nn.LeakyReLU())
        self.module.add_module("dropout_1_dec", nn.Dropout(0.5))
        self.module.add_module("linear_2_dec", nn.Linear(1024, X_shape))
...
class Regressor_SolverBig(nn.Module):
    """
    This class defines a regressor, in the CovMat and also in de LMq and LZ. 
    Depending on the paramter softmax it apply or not this function (It is used only in the CovMat case.)
    This is exactly the same as the QM in HistNetQ.
    """
    def __init__(self, input_dim, output_dim, dropout):
        super(Regressor_SolverBig, self).__init__()
        self.module = torch.nn.Sequential()
        self.module.add_module("linear_1", nn.Linear(input_dim, 4096))
        self.module.add_module("relu_1", nn.LeakyReLU())
        self.module.add_module("dropout_1", nn.Dropout(0.5))
        self.module.add_module("linear_2", nn.Linear(4096, output_dim))
...
def parse_arguments():
    ...
    parser.add_argument("-sym", "--sym", type=bool, default=False, help = "Use Bags labeled by prevalence as training.")
    
    return parser.parse_args()

def load_dataset(dataset_name, symmetric=False):
    # LeQua2022 
    ...
    # LeQua2024
        ...
        if symmetric:
            return val_std, test_std, val_std, train.n_classes 
...
def initialize_model_components(args, train, num_classes, patience=20):
    
    #CAMBIO
    if isinstance(train, AbstractProtocol):
        s, _ = next(train())
        n_feat = s.shape[1]
    else: 
    ...
    if args.phi in phis:
        if args.phi == "phi_h":
            ...
            elif args.dataset in LEQUA:
                instance_representation = InstanceRepresentationBig(n_feat, dropout, return_next_to_last=False, num_classes=num_classes)
            else:
            ...
            if args.dataset in IMAGES:
                ...
                img_size = train.X.shape[2]
                n_feat = 32
                instance_representation = InstanceRepresentationImages(in_channels, n_feat, return_next_to_last=True, img_size=img_size)
            elif args.dataset in LEQUA:
                instance_representation = InstanceRepresentationBig(n_feat, dropout, return_next_to_last=False, num_classes=num_classes)
            else:
            ...
            if args.dataset in IMAGES:
                ...
                instance_representation = InstanceRepresentationImages(in_channels, n_feat, return_next_to_last=True, img_size=img_size)
                latent_dims = instance_representation.dimensions() 
                instance_representation_decoder = InstanceRepresentationImages_decoder(latent_dims, in_channels, img_size)
            elif args.dataset in LEQUA:
                instance_representation = InstanceRepresentationBig(n_feat, dropout, return_next_to_last=False, num_classes=num_classes)
                latent_dims = instance_representation.dimensions()
                instance_representation_decoder = InstanceRepresentationBig_decoder(n_feat, latent_dims, dropout)
            else:
                instance_representation = InstanceRepresentation(n_feat, dropout, return_next_to_last=True)
                latent_dims = instance_representation.dimensions()
    ...
    if args.Phi in Phis:
        if args.Phi == "Phi_mu":
            ...
            if args.dataset in LEQUA or args.dataset in IMAGES:
                regressor_cm = Regressor_SolverBig((latent_dims*(latent_dims+1)//2)+latent_dims, latent_dims, dropout)
            else:
    ...
    if args.L in losses:
        if args.L == "L2":
            ...
            if args.Phi == "Phi_H":
                if args.dataset in LEQUA or args.dataset in IMAGES:
                    regressor = Regressor_SolverBig(input_dim=(latent_dims*num_classes+latent_dims)*n_bins, output_dim=num_classes, dropout=dropout)
                else:
                    regressor = Regressor_Solver(input_dim=(latent_dims*num_classes+latent_dims)*n_bins, output_dim=num_classes, dropout=dropout)
            elif args.Phi == "Phi_G":
                if args.dataset in LEQUA or args.dataset in IMAGES:
                    regressor = Regressor_SolverBig(input_dim=n_gaussians*num_classes+n_gaussians, output_dim=num_classes, dropout=dropout)
                else:
                    regressor = Regressor_Solver(input_dim=n_gaussians*num_classes+n_gaussians, output_dim=num_classes, dropout=dropout)
            else:
                if args.dataset in LEQUA or args.dataset in IMAGES:
                    regressor = Regressor_SolverBig(input_dim=latent_dims*num_classes+latent_dims, output_dim=num_classes, dropout=dropout)
                else:
            ...
            if args.Phi == "Phi_H":
                if args.dataset in LEQUA or args.dataset in IMAGES:
                    regressor = Regressor_SolverBig(input_dim=latent_dims*n_bins, output_dim=num_classes, dropout=dropout)
                else:
                    regressor = Regressor_Solver(input_dim=latent_dims*n_bins, output_dim=num_classes, dropout=dropout)
            elif args.Phi == "Phi_G":
                if args.dataset in LEQUA or args.dataset in IMAGES:
                    regressor = Regressor_SolverBig(input_dim=n_gaussians, output_dim=num_classes, dropout=dropout)
                else:
                    regressor = Regressor_Solver(input_dim=n_gaussians, output_dim=num_classes, dropout=dropout)
            else:
                if args.dataset in LEQUA or args.dataset in IMAGES:
                    regressor = Regressor_SolverBig(input_dim=latent_dims, output_dim=num_classes, dropout=dropout)
                else:
...
def train_model(args, train, val, test, optimizer, scheduler, early_stopping, instance_representation, bag_representation, regressor_cm, histnet, gmnet, regressor, linear_classif, instance_representation_decoder, use_Mq, solver, num_classes):
    ...
    report_dir = os.path.join(f"results_LEQUA/reports_{args.seed}", args.dataset)
    os.makedirs(report_dir, exist_ok=True)
    report_path = os.path.join(f"results_LEQUA/reports_{args.seed}", args.dataset, args.phi+'_'+args.Phi+'_'+args.L+'_'+args.error+('_dropout' if args.dropout != 0.0 else ''))
    ...
    for epoch in range(n_epochs):
        ...
        for i, data in enumerate(upp_gen()):
            ...
            if use_Mq:
                ...
                dev = next(bag_representation.parameters()).device
                if args.sym:
                    X = torch.tensor(data[0], dtype=torch.float32, device=dev)
                    prev = torch.as_tensor(data[1], dtype=torch.float32, device=dev)
                else:
...
if __name__ == '__main__':
    ...
    if USE_WANDB:
        wandb.login()
        wandb_project_name = f"RLQ_LEQUA_{args.seed}"
        wandb_execution_name = args.dataset+'_'+args.phi+'_'+args.Phi+'_'+args.L+'_'+args.error+'_lr'+args.lr+'_nbags'+args.b+('_symetric' if args.sym == True else '')+('_dropout' if args.dropout != 0.0 else '')
        wandb.init(
    ...
    train, test, val, num_classes = load_dataset(args.dataset, args.sym)
    
    optimizer, scheduler, early_stopping, instance_representation, bagRepresentation, regressor_cm, histnet, gmnet, regressor, linear_classif, instance_representation_decoder, use_Mq, solver = initialize_model_components(args, train, num_classes, patience=40)
        
    train_model(args, train, val, test, optimizer, scheduler, early_stopping, instance_representation, bagRepresentation, regressor_cm, histnet, gmnet, regressor, linear_classif, instance_representation_decoder, use_Mq, solver, num_classes)
